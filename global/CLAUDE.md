...
| Code review | `/codex:review` and `/codex:adversarial-review` are **user-typed only** (`disable-model-invocation: true`; the plugin's agent contract also bars its subagent from those subcommands, so an agent does not call `codex.sh review` / `codex.sh adversarial-review` via Bash). **The agent route for a review is `/codex:rescue` read-only** — the plugin's contract names "review, diagnosis, or research without edits" as `task` work — with the review contract in the task text (`branch-completion-review.md` Stage 2). Either route discharges the stage; the slash command is the user's to type |
