...
|branch-completion-review.md: Adversarial GO/NO-GO, triggered by RISK CLASS not diff size (lifetime/cancellation · persistence/format · integrity of a check OR the policy behind it, incl. that rule itself · trusted external surface · real-time/hardware); trigger must be machine-checked where a gate exists; refactor pass demoted to optional, triggered by fan-out authoring; adversary runs on `codex.sh adversarial-review` (the slash command is user-typed only); ONE review pass per branch (never stop-gate + adversary); project files reference, never restate — source trees (src/app/apps/packages/lib/Sources), .claude/{skills,rules}, .github
...
| Code review | `/codex:review` (defect pass) and `/codex:adversarial-review` (challenges the approach) — use these in place of a Claude-side review pass. **Both slash commands are `disable-model-invocation: true` (user-typed only).** The agent-callable form is the companion subcommand: `~/.claude/scripts/codex.sh review` / `~/.claude/scripts/codex.sh adversarial-review --wait --base origin/<integration> <focus>` via Bash. A rule or skill that tells the orchestrator to run the slash command parks the work on the user |
...
| `/code-review`, `/security-review`, a branch-completion review pass | `codex.sh review` / `codex.sh adversarial-review` (the slash commands are user-typed only — see § Codex-first) |
