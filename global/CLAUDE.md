...
|branch-completion-review.md: Adversarial GO/NO-GO, triggered by RISK CLASS not diff size (lifetime/cancellation · persistence/format · integrity of a check OR the policy behind it, incl. that rule itself · trusted external surface · real-time/hardware); trigger must be machine-checked where a gate exists; refactor pass demoted to optional, triggered by fan-out authoring; adversary is the Claude subagent (agent-callable) or user-typed `/codex:adversarial-review` — never the companion via Bash; ONE review pass per branch (never stop-gate + adversary); project files reference, never restate — source trees (src/app/apps/packages/lib/Sources), .claude/{skills,rules}, .github
...
| Code review | `/codex:review` (defect pass) and `/codex:adversarial-review` (challenges the approach) are **user-typed only**: both carry `disable-model-invocation: true`, and the harness refuses the Skill route with "do not replicate this skill's workflow by other means". An agent does not call `codex.sh review` / `codex.sh adversarial-review` through Bash either — that is the replication the refusal names. Agent-run review stays on the Claude column (`branch-completion-review.md` Stage 2); to spend Codex on a review, the user types the command, and either route discharges the stage |
...
| `/code-review`, `/security-review`, a branch-completion review pass | **Exception, not a route**: Codex review is user-invoked only (`disable-model-invocation`). When the user is present, ask them to type `/codex:review` / `/codex:adversarial-review`; otherwise the review runs on the Claude column — one of the few things that legitimately stays here |
