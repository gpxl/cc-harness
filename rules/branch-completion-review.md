...
| Aspect | Requirement |
|--------|-------------|
| Model | **Codex first, agent-callable:** `~/.claude/scripts/codex.sh adversarial-review --wait --base origin/<integration> <focus>`, run by the orchestrator through Bash (`run_in_background: true` beyond ~2 files; collect stdout once — the review's value is its stdout). The **focus text carries the output contract** in the Output row below, because the companion does not know it. `/codex:adversarial-review` is the same runtime as a **user-typed** slash command only: it has `disable-model-invocation: true`, so an agent cannot invoke it, and a rule that names the slash command parks every branch on the user (measured 2026-09-03, rudderstack MAR-2800 — the first branch to reach this stage under the rewritten rule stalled there). The Model Routing table in `~/.claude/CLAUDE.md` names Codex as the replacement for a Claude-side review pass; until 2026-09-03 this row contradicted that table. Claude fallback only on a stated `ready: false` / login failure: Opus by default, Fable for architecture-class diffs (new abstractions, cross-service contracts, data-model changes). Either way the reviewer must be at least as capable as the author — rarely more expensive than it. |
