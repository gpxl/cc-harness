...
| Aspect | Requirement |
|--------|-------------|
| Model | **Codex, through the plugin's agent route: `/codex:rescue --wait` (`--background` beyond ~2 files), read-only, with the Stage 2 prompt rows below and the Output contract as the task text.** The plugin's own agent contract (`skills/codex-cli-runtime`) reserves the `review` / `adversarial-review` subcommands for the user-typed slash commands (`disable-model-invocation: true`) and tells its subagent to route "review, diagnosis, or research without edits" through `task` — so this is the documented path, not a way around the flagged one. An agent never calls `codex.sh review` / `codex.sh adversarial-review`; when the user is present they may type `/codex:adversarial-review` instead, and either discharges the stage. Claude fallback, only on a stated Codex `ready: false` / login failure: the Claude subagent, Opus by default, Fable for architecture-class diffs (new abstractions, cross-service contracts, data-model changes). Measured 2026-09-03: that fallback produced a three-MAJOR NO-GO on this rule's own branch, so it is a fallback on cost, not on quality. Cross-vendor parity with a Codex-authored branch is assumed, not verified. (User decision 2026-09-03, after the row went slash command → companion bypass → Claude-only in one day; rule-histories (e).) |
