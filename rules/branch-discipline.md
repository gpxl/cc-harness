...
Rules steer the model. Hooks are a hard stop at tool-call time. Use both: this rule explains intent and recovery; the hook enforces the rule when intent fails. They complement each other — see AudioWebsite's `scripts/branch-guard.sh` and `.claude/rules/branching.md` for a live example.
