...
`task` invocation) reports one of six states, not a binary done/not-done. Misreading `unknown`
...
6. **`running` is not a deliverable.** Claim completion only once `/codex:status` (or
   `/codex:result`) reports an actual `completed` (or a terminal `failed`/`cancelled`)
   outcome — never report progress-in-flight as if the work were done.
